# -*- coding: utf-8 -*-
# @Date : 2019/09/18
# @Author : water
# @Desc  :将模板内容填入TEMPLATE，自动生成json格式的"template_single.temp"
# @Version  : v1.0

import json,os


TEMPLATE_NAME = "template_single.temp"
TEMPLATE_PATH =os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),"data",TEMPLATE_NAME)
TEMPLATE = {
    "FPXX": {
        "FP_KJ": {
            "FPQQLSH": "10780194078",
            "XSF_NSRMC": "赤井公司",
            "XSF_NSRSBH": "110101N1KRX0F08",
            "FPDM": "241909000240",
            "FPHM": "19081601",
            "KPRQ": "2019-08-11 14:17:39",
            "KPLX": "2",
            "KPHJJE": "1199.00",
            "HJBHSJE": "1024.79",
            "KPHJSE": "174.21",
            "KPXM": "宇宙飞船",
            "GMF_NSRSBH": "110101009791112",
            "GMF_NSRMC": "柯南公司",
            "GMF_YH": "12310",
            "GMF_YHZH": "123",
            "GMF_DZ": "北京宇宙区",
            "GMF_DH": "15588888884",
            "GMF_SF": "1239",
            "GMF_EMAIL": "81924573@qq.com",
            "GMF_SJ": "15588888884",
            "YFPHM": "1237",
            "YFPDM": "1238",
            "JQBH": "661505060904",
            "KPY": "开票员",
            "SKY": "收款员",
            "FHR": "复核人",
            "SWJG_DM": "111019201",
            "SPHSL": "1",
            "FP_ZLDM": "2",
            "VERSION": "1",
            "XSF_DZ": "北京市宇宙区外星人1号",
            "XSF_DH": "123123123",
            "XSF_YH": "1236",
            "XSF_YHZH": "招商银行",
            "FJH": "9",
            "DSPTBM": "11110101",
            "DKBZ": "0",
            "PDFPATH": "/home/gin/",
            "TSCHBZ": "1",
            "CHYY": "冲红原因",
            "XHQD": "销货清单",
            "XHQDBZ": "0",
            "BMB_BBH": "1233",
            "QD_BZ": "0",
            "DDH": "11111111117",
            "DSF_PTBM": "11111111",
            "SGBZ": "1",
            "QDXMMC": "1232",
            "SKM": "12323354657677",
            "EWM": "",
            "BZ": "1",
            "FP_MW": "6134*3>950+75<8/1208-85*</*14>-65-142/+7390*24855672678136749+92184+/8*553-9-8-+5*</*1/670433467<<3<0*24<4*6",
            "DDSJ": "2019-08-01 14:15:11",
            "JYM": "65560891271390877510",
			"ISCOVER":"1"
        },
        "FP_KJ_MX": [
            {
                "SPHXH": "01",
                "SPMC": "华为手机",
                "SPSL": "1.00000000",
                "SPJE": "1024.79",
                "SPDJ": "1024.78632479",
                "DW": "1231",
                "GGXH": "12",
                "HSJBZ": "0",
                "KCE": "2",
                "SE": "174.21",
                "SL": "17%",
                "SPBM": "123",
                "ZXBM": "1234",
                "YHZCBS": "1",
                "LSLBS": "0",
                "ZZSTSGL": "1235",
                "FPHXZ": "1"
            }
        ],
        "FP_WLXX": [
            {
                "CYGS": "承运公司",
                "WLDH": "123123123",
                "SHDZ": "测试送货地址",
                "SHSJ": "2019-08-01 14:17:11"
            }
        ],
        "FP_ZFXX": {
            "ZFFS": "测试支付方式",
            "ZFPT": "测试支付平台",
            "ZFLSH": "12312312367"
        }
    }
}


def temp_to_json(template,template_path):
    with open(template_path, "w") as f:
        json.dump(template, f,ensure_ascii=False)
        print("template生成成功！！！")


if __name__ == "__main__":
    temp_to_json(TEMPLATE,TEMPLATE_PATH)